...
# MOMO 전용 기본값
# - 수익은 더 크게 노리고
# - Trailing 폭은 REV/MS보다 더 넓게 허용
DEFAULT_EXIT_PARAMS_MOMO: Dict[str, Any] = {
    "tp_rate": 0.03,                 # 3% 정도부터 본격 TP 영역
    "sl_rate": -0.02,                # 손절은 동일 -2%
    "max_hold_bars": 36,             # 3시간까지 홀딩 허용

    "min_hold_bars_for_timeout": 6,
    "timeout_deadband": 0.015,       # ±1.5% 이내면 TIMEOUT 컷

    # 모멘텀은 Trailing이 핵심
    "use_trailing": True,
    "tp_start_rate": 0.03,           # 3% 이상부터 Trailing Zone
    "trail_gap": 0.015,               # 최고 수익 대비 1.5% 되돌림이면 컷
    "min_bars_for_trailing": 0,      # 최소 3봉 이후부터 Trailing

    # ML EXIT: 수익 중/손실 중 동적으로 관리
    "use_ml_exit": True,
    "ml_recheck_min_bars": 6,
    "ml_recheck_interval": 3,
}


def decide_exit_momo(
    pos: CrPosition,
    df_5m: pd.DataFrame,
    cur_price: float,
    now_dt: datetime,
    ...
    if params is None:
        ...
        params.update(DEFAULT_EXIT_PARAMS_MOMO)
    else:
        merged = DEFAULT_EXIT_PARAMS_BASE.copy()
        merged.update(DEFAULT_EXIT_PARAMS_MOMO)
    ...
    tp_rate = float(params.get("tp_rate", 0.05))
    ...
    # 4) Trailing TP (모멘텀 핵심)
    if params.get("use_trailing", True):
        min_trail_bars = int(params.get("min_bars_for_trailing", 3))
    ...
    min_bars = int(params.get("ml_recheck_min_bars", 6))
