...
# 주식(KR/US) 전용 백테스트
# -------------------------------------------------------
def run_backtest_for_stock_universe(
    universe,
    *,
    ...
    """
    주식(KR / US) 유니버스에 대해 백테스트 전체를 수행하고
    backtests 테이블에 결과를 저장하는 함수.

    - universe: [{"region": "...", "symbol": "...", "excd": "..."}, ...]
    - model_setting_key: settings 테이블에 저장한 모델 경로 키 이름
      예) "active_model_path_kr", "active_model_path_us", "active_model_path_stock"
    - note_prefix: backtests.note 에 붙일 접두어 (예: "[KR]" "[US]" "[STOCK]")
    ...
    for t in universe:
        ...
        # 주식 전용: 코인(region == "CR" 등)은 스킵
        if region not in ("KR", "US"):
            print(f"[SKIP] 주식 백테스트에서 제외된 region: {region} {symbol}")
            continue
