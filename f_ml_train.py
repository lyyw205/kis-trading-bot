...
# 2) 주식(KR/US) 전용 학습 함수
# -----------------------------------------------------------
def train_seq_model_for_stock_universe(
    universe: list[dict],
    *,
    region_filter: str | None,  # "KR", "US", 또는 None(둘 다)
    model_setting_key: str,
    note_prefix: str = "[STOCK] ",
    model_dir: str = "models_stocks",
    extra_config: dict | None = None,
):
    """
    주식 유니버스(KR / US)에 대해 모델을 학습하고 저장하는 전용 함수.

    - universe: [{"region": "KR"/"US", "symbol": "...", "excd": "...", ...}, ...]
    - region_filter:
        - "KR": 코스피/코스닥 전용 모델
        - "US": 미국주식 전용 모델
        - None: KR + US 전체를 한 모델로 학습
    - model_setting_key: settings에 저장할 키 (예: active_model_path_kr, active_model_path_us)
    - note_prefix: 로그/파일명 접두어 (예: "[KR_STOCK] ")
    - model_dir: 주식 모델 저장 디렉토리 (기본: models_stocks)
    - extra_config: 전략 버전 등 추가 메타데이터 dict
    """
    os.makedirs(model_dir, exist_ok=True)
    db = BotDatabase(DB_PATH)
    db.log(f"{note_prefix}🧠 [STOCK] 시퀀스 기반 ML 모델 학습 시작 (setting_key={model_setting_key})")
    ...
    # 1-1) 주식 전용 region 필터링 (KR / US 만 사용)
    df_samples = df_samples[df_samples["region"].isin(["KR", "US"])].copy()
    if df_samples.empty:
        print("[STOCK] KR/US 샘플이 없습니다.")
        return

    # 선택적 region_filter (KR 전용 / US 전용 모델 분리 학습할 때)
    if region_filter is not None:
        if region_filter not in ("KR", "US"):
            print(f"[STOCK] region_filter는 KR 또는 US 또는 None만 허용됩니다. (입력: {region_filter})")
            return

        before = len(df_samples)
        df_samples = df_samples[df_samples["region"] == region_filter].copy()
        after = len(df_samples)
        print(f"[STOCK] region={region_filter} 필터: {before} → {after}")
        if df_samples.empty:
            print(f"[STOCK] region={region_filter} 에 해당하는 샘플이 없습니다.")
            return
    ...
    print(f"[STOCK] UNIVERSE 필터 전 샘플 수: {before_cnt}")
    print(f"[STOCK] UNIVERSE 필터 후 샘플 수: {after_cnt}")

    if df_samples.empty:
        print("[STOCK] UNIVERSE에 해당하는 샘플이 없습니다.")
        return
    ...
    if not X_list:
        print("[STOCK] 유효한 피처를 가진 샘플이 없습니다.")
        return

    X = np.array(X_list, dtype=float)
    y = np.array(y_list, dtype=int)

    print(f"[STOCK] 학습에 사용되는 샘플 수: {len(X)}")
    print(f"[STOCK] 스킵된 샘플 수: {skip_count}")
    print(f"[STOCK] 피처 차원: {X.shape[1]}")
    ...
    print(f"{note_prefix}=== [STOCK] Classification Report ===")
    print(classification_report(y_test, y_pred, digits=4))

    print(f"{note_prefix}=== [STOCK] Confusion Matrix ===")
    print(confusion_matrix(y_test, y_pred))

    accuracy = float((y_pred == y_test).mean())
    print(f"{note_prefix}[STOCK] Validation Accuracy: {accuracy:.4f}")
    ...
    model_filename = f"seq_model_stock_{tag}{version_str}.pkl"
    ...
    print(f"{note_prefix}[STOCK] 모델 저장 완료: {model_path}")
    print(f"{note_prefix}[STOCK] CONFIG_HASH: {config_hash}")
    ...
    try:
        ...
        print(f"{note_prefix}[STOCK] 메타데이터 저장 완료: {meta_path}")
    except Exception as e:
        print(f"{note_prefix}[STOCK] 메타데이터 저장 실패: {e}")
    ...
    try:
        ...
        conn.commit()
        conn.close()
        db.log(f"{note_prefix}[STOCK] models 테이블에 버전 기록 완료: {model_path}")
    except Exception as e:
        db.log(f"{note_prefix}[STOCK] models 테이블 기록 실패: {e}")

    # 10) settings 에 model_setting_key 갱신
    try:
        db.set_setting(model_setting_key, model_path)
        db.log(f"{note_prefix}[STOCK] {model_setting_key} 갱신: {model_path}")
    except Exception as e:
        db.log(f"{note_prefix}[STOCK] {model_setting_key} 갱신 실패: {e}")

    db.log(
        f"{note_prefix}✅ [STOCK] 시퀀스 기반 ML 모델 학습/저장 완료 "
